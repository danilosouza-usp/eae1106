...
\tightlist
\item
  \textbf{06 de Abril, 2026}: Correção de typos nos primeiros 6
  capítulos; inclusão de novos exercícios; inclusão da versão inicial do
  capítulo 7.
\item
...
\begin{tcolorbox}[enhanced jigsaw, breakable, coltitle=black, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Importante}, titlerule=0mm, colbacktitle=quarto-callout-important-color!10!white, opacitybacktitle=0.6, arc=.35mm, bottomrule=.15mm, toptitle=1mm, opacityback=0, colback=white, bottomtitle=1mm, toprule=.15mm, rightrule=.15mm, left=2mm, colframe=quarto-callout-important-color-frame, leftrule=.75mm]
...
\begin{tcolorbox}[enhanced jigsaw, breakable, coltitle=black, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{O que significam as especificações do seu computador?}, titlerule=0mm, colbacktitle=quarto-callout-tip-color!10!white, opacitybacktitle=0.6, arc=.35mm, bottomrule=.15mm, toptitle=1mm, opacityback=0, colback=white, bottomtitle=1mm, toprule=.15mm, rightrule=.15mm, left=2mm, colframe=quarto-callout-tip-color-frame, leftrule=.75mm]
...
\item
  ...
  \end{enumerate}
\item
  Para que computadores possam armazenar e manipular textos, é
  necessário representar caracteres (letras, números, símbolos) por meio
  de números. O ASCII (\emph{American Standard Code for Information
  Interchange}) é um dos primeiros e mais importantes padrões de
  codificação de caracteres. Nesse padrão, cada caractere é representado
  por um número inteiro entre 0 e 127. Esse número é armazenado usando 7
  bits (embora, na prática, seja guardado em 1 byte = 8 bits). Por
  exemplo, a letra ``A'' corresponde ao número 65. A letra ``a''
  corresponde ao número 97. O número ``0'' corresponde ao código 48. Com
  base nessa ideia de representação digital da informação, responda à
  questão abaixo.
\end{enumerate}

Um arquivo de texto possui 1 milhão de caracteres ASCII.

\begin{enumerate}
\def\labelenumi{\alph{enumi}.}
\tightlist
\item
  Quantos bytes ele ocupa? Quantos bits?
\item
  Se comprimido para 40\% do tamanho original, qual será o novo tamanho?
\item
  Explique por que arquivos de texto costumam ocupar um pequena fração
  do seu tamanho original quando comprimidos.
\end{enumerate}
...
Em algumas outras linguagens, o símbolo de acento circunflexo é usado
para a exponenciação, mas no Python é um operador \emph{bitwise},
chamado XOR. Se não tiver familiaridade com operadores bitwise, o
resultado o surpreenderá. Não abordaremos operadores bitwise neste
curso, mas você pode ler sobre eles em
...
\def\labelenumi{\arabic{enumi}.}
\item
  Discuta por que Python se tornou tão popular em ciência de dados e
  aprendizado de máquina, apesar de não ser a linguagem mais rápida em
  execução. Cite outros contextos em que Python pode não ser a linguagem
  mais adequada.
\item
  ...
  \item
    ...
    \emph{pace} médio (tempo por quilômetro em minutos e segundos)?
  \end{enumerate}
\item
...
\item
  Leia o
  \href{https://www.freecodecamp.org/news/programming-naming-conventions-explained/}{artigo}
  que versa sobre convenções de nomes em programação. Qual é a
  importância de se ter um padrão de nomeação de variáveis (pense
  especialmente em ambientes de trabalho com diversos usuários
  manipulando as mesmas variáveis e o mesmo código), tanto na forma
  quanto no conteúdo das palavras?
\end{enumerate}
...
\begin{tcolorbox}[enhanced jigsaw, breakable, coltitle=black, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Curiosidade}, titlerule=0mm, colbacktitle=quarto-callout-note-color!10!white, opacitybacktitle=0.6, arc=.35mm, bottomrule=.15mm, toptitle=1mm, opacityback=0, colback=white, bottomtitle=1mm, toprule=.15mm, rightrule=.15mm, left=2mm, colframe=quarto-callout-note-color-frame, leftrule=.75mm]
...
\item
  ...
  não há garantia de respostas \(100\%\) corretas. Além disso, há um
  grau inerente de imprevisibilidade: pequenas variações no prompt podem
  gerar respostas diferentes, e o próprio modelo é não determinístico.
  Isso significa que, mesmo com a mesma pergunta, resultados distintos
  podem surgir. Para aplicações que exigem precisão rigorosa e
  consistência absoluta, essa limitação deve ser considerada
  explicitamente.
\item
  ...
  posteriormente.
\item
  \textbf{Dados da aplicação são sensíveis ou protegidos?} O
...
\item
  ...
  (i) seja descritivo, (ii) evite ambiguidades e informações
...
\item
  Você quer que um modelo do tipo LLM, tal qual o ChatGPT, escreva um
  trecho de código em Python que leia um arquivo CSV e calcule a média
  de uma coluna chamada ``preco'', retornando uma mensagem formatada com
  duas casas decimais. Escreva um prompt claro e específico para
  solicitar isso a um modelo de linguagem, incluindo:
\end{enumerate}
...
\item
  um exemplo simples da saída esperada (por exemplo: ``A média de preço
  é R\$ 23.45'').
\end{itemize}
...
\begin{verbatim}
<zip object at 0x00000155CD102040>
\end{verbatim}
...
\item
  ...
  de cada variável. Explique por que a expressão \texttt{a\ +\ e} gera
...
\item
  Por quê tuplas podem ser chaves de dicionário e listas não?
\end{enumerate}
...
\begin{verbatim}
Essa é a iteração 0 e o número sorteado foi: 0.47
Essa é a iteração 1 e o número sorteado foi: 0.71
Essa é a iteração 2 e o número sorteado foi: 0.03
Essa é a iteração 3 e o número sorteado foi: 0.76
Essa é a iteração 4 e o número sorteado foi: 0.79
Essa é a iteração 5 e o número sorteado foi: 0.60
Essa é a iteração 6 e o número sorteado foi: 0.60
Essa é a iteração 7 e o número sorteado foi: 0.71
Essa é a iteração 8 e o número sorteado foi: 0.32
Essa é a iteração 9 e o número sorteado foi: 0.44
\end{verbatim}
...
\begin{verbatim}
Tempo necessário com list comprehensions: 0.0972 segundos
Tempo necessário com loop: 0.1272 segundos

A abordagem de loop demorou 30.9% mais tempo!
\end{verbatim}
...
\begin{tcolorbox}[enhanced jigsaw, breakable, coltitle=black, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Teorema Central do Limite}, titlerule=0mm, colbacktitle=quarto-callout-tip-color!10!white, opacitybacktitle=0.6, arc=.35mm, bottomrule=.15mm, toptitle=1mm, opacityback=0, colback=white, bottomtitle=1mm, toprule=.15mm, rightrule=.15mm, left=2mm, colframe=quarto-callout-tip-color-frame, leftrule=.75mm]
...
\begin{verbatim}
[ 22.77834     13.63694085  16.75798023 -29.60992784 -34.96779749
  17.51442948   0.6093497    8.95242978  30.30652745   8.58616112]
\end{verbatim}
...
\begin{verbatim}
[-33.98781534 -19.82934896 -14.840974    17.49112371  15.67365607
 -16.02678993   9.81117031  15.081256   -18.96158384  17.1798399 ]
\end{verbatim}
...
\item
  Escreva um \emph{loop} que parta de um número inteiro positivo e
  retorne ao fim do código a quantidade de dígitos pares que existem no
  número inicial.
\item
...
\tightlist
\item
  Escreva uma função chamada triângulo que recebe uma string como
  argumento e retorna como output um triângulo retângulo de determinada
  altura, usando cópias da string, como no exemplo:
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{triangle(}\StringTok{\textquotesingle{}L\textquotesingle{}}\NormalTok{, }\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\setcounter{enumi}{1}
\item
  Defina uma função que retorne a quantidade de dinheiro em uma conta
  corrente após uma transação. Os dois parâmetros para essa função são
  saldo e transação. Ambos serão argumentos inteiros. O saldo é quanto
  dinheiro existe atualmente na conta, e a transação quanto adicionar ou
  subtrair da conta corrente (baseado se é um inteiro positivo ou
  negativo). Note que essa operação é mais complicada que apenas
  \texttt{saldo\ +\ transacao}. Isso porque se a transação for negativa
  (saída de dinheiro da conta) e em valor absoluto maior do que o saldo
  existente, a transação deve ser ignorada e o valor original de saldo
  retornado pela função.
\item
  Escreva uma função chamada \texttt{sum\_list} que recebe uma lista de
  números e retorna um dicionário com duas chaves e dois valores.

  \begin{itemize}
  \tightlist
  \item
    O primeiro par chave-valor deve associar à chave \texttt{soma} o
    valor numérico da soma de todos os elementos.
  \item
    O segundo par chave-valor deve associar à chave \texttt{elementos} o
    número de elementos somados.
  \end{itemize}
\item
  Defina uma função que recebe uma temperatura em Celsius e retorna o
  valor convertido para Fahrenheit com duas casas decimais. Agora crie
  uma segunda função, mais genérica, que receba como primeiro argumento
  \texttt{CtoF} ou \texttt{FtoC} e como segundo argumento a temperatura.
  Essa segunda função deve ser capaz de converter as escalas de
  temperatura em ambas as direções. Por padrão, o primeiro argumento
  deve ser definido como \texttt{CtoF}.
\end{enumerate}
...
té aqui, os programas escritos consistem em sequências de instruções,
condicionais e laços que manipulam objetos como números, listas e
dicionários. Essa forma é suficiente para resolver problemas simples,
mas rapidamente se torna difícil de manter quando o código cresce ou
quando a mesma lógica precisa ser reutilizada várias vezes.
...
O foco deste capítulo será o pacote \emph{NumPy}, que, apesar de já vir
instalado com o Anaconda, não é nativo do Python e, portanto, é preciso
carregá-lo. Além disso, em alguma aplicações utilizaremos o pacote
\emph{time} para contagem de tempo de máquina e o pacote \emph{SymPy}
...
Tempo necessário para a realização dos cálculos utilizando listas: 0.2862 segundos
