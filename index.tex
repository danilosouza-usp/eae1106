...
\includegraphics[width=7.29167in,height=\textheight,keepaspectratio]{images/old_desktop.jpg}
...
\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, colbacktitle=quarto-callout-important-color!10!white, toptitle=1mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Importante}, coltitle=black, breakable, bottomrule=.15mm, opacitybacktitle=0.6, colback=white, left=2mm, bottomtitle=1mm, colframe=quarto-callout-important-color-frame, arc=.35mm, rightrule=.15mm, opacityback=0, toprule=.15mm, leftrule=.75mm]
...
\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{O que significam as especificações do seu computador?}, coltitle=black, breakable, bottomrule=.15mm, opacitybacktitle=0.6, colback=white, left=2mm, bottomtitle=1mm, colframe=quarto-callout-tip-color-frame, arc=.35mm, rightrule=.15mm, opacityback=0, toprule=.15mm, leftrule=.75mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{images/terminal_windows.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/terminal_macos.jpeg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/cmd1.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/cmd2.jpg}}
...
Neste curso, utilizaremos Python, uma linguagem de programação de alto
nível, interpretada, multiparadigma e de tipagem dinâmica e forte.
Python é amplamente utilizada em análise de dados, ciência de dados,
automação, aprendizado de máquina e pesquisa empírica em Economia. Seu
principal atrativo é permitir que o aluno foque na lógica do problema, e
não em detalhes de implementação de baixo nível, além de contar com um
ecossistema extenso de bibliotecas científicas. No próximo capítulo,
deixaremos o nível conceitual e começaremos a programar de fato:
escrever código, executar comandos, interpretar erros e manipular dados
...
\includegraphics[width=7.29167in,height=\textheight,keepaspectratio]{images/clayton_coding.png}
...
Neste capítulo aprenderemos como instalar o Python e outros pacotes. Ele
foi escrito para que o aluno que acompanhou e absorveu o conteúdo do
capítulo anterior, sobre fundamentos de computação, tenha capacidade de
acompanhar.

\section{O Python}\label{o-python}

Python foi criado no final dos anos oitenta(1989) por Guido van Rossum
no Centro de Matemática e Tecnológia da Informação (CWI, Centrum
Wiskunde e Informatica), na Holanda, como sucessor da linguagem de
programação ABC, capaz de lidar com exceções e interagir com o sistema
operacional Amoeba. O nome da língua vem do gosto de seu criador pelos
humoristas britânicos Monty Python (fonte:
https://blog.vulpi.com.br/python-como-surgiu/)

\subsection{Porque escolher}\label{porque-escolher}
...
da distribuição Anaconda (https://www.anaconda.com/). Além de ser
gratuita e conter vários dos pacotes que iremos utilizar, ela nos
fornece diversas ferramentas que facilitam nossa interação com a
linguagem. É hoje uma das distribuições do Python mais populares no
mundo! \textbf{Recomendo fortemente utilizar esse caminho}.
...
\item
  ...
  no site do Anaconda (https://www.anaconda.com/download), fornecer um
  email para cadastro e baixar o arquivo executável necessário para a
  instalação a depender do seu sistema operacional.
\end{enumerate}
...
\item
  ...
  escolhendo a opção de instalar para todos os usuários do computador, o
  que requer acesso de administrador.
\end{enumerate}
...
conhecimento acerca da utilização correta de terminais (e.g,
\emph{prompt de comando} do Windows). Ao abrir o Anaconda Navigator no
Windows, você deve ver algo assim
...
mais, esse
\href{https://anaconda.cloud/getting-started-with-anaconda-distribution?src=share}{vídeo}
é um bom começo.

\subsection{IDEs}\label{ides}

IDE é um acrônimo para Integrated Development Environment, em português,
Ambiente de Desenvolvimento Integrado. Ele é um programa que reúne
ferramentas necessárias para a construção de outros softwares. A
utilização de um IDE ajuda muito os programadores e empresas, pois torna
mais rápido o desenvolvimento de aplicações, aumentando a produtividade
e reduzindo custos. Existem IDE's específicas para plataformas, e outras
que são mais flexíveis. As mais famosas são o Sublime Text e o Visual
Studio Code.
...
Tradicionalmente, o primeiro programa que se escreve em uma nova
linguagem chama-se ``Hello, World!'', porque tudo o que faz é exibir as
palavras ``Hello, World!'' na tela. No Python, ele se parece com isto:
...
print é uma função. No Python 2, a instrução print é ligeiramente
diferente; ela não é uma função, portanto não usa parênteses.

\subsection{Operadores aritméticos}\label{operadores-aritmuxe9ticos}

Depois do ``Hello, World'', o próximo passo é a aritmética. O Python tem
operadores, que são símbolos especiais representando operações de
computação, como adição e multiplicação. Os operadores +, - e * executam
a adição, a subtração e a multiplicação, Finalmente, o operador **
executa a exponenciação; isto é, eleva um número a uma potência, como
...
no Python é um operador bitwise, chamado XOR. Se não tiver familiaridade
com operadores bitwise, o resultado o surpreenderá. Não abordaremos
operadores bitwise neste curso, mas você pode ler sobre eles em
...
Um valor é uma das coisas básicas com as quais um programa trabalha,
como uma letra ou um número. Alguns valores que vimos até agora foram 2,
42.0 e `Hello, World!'.

Esses valores pertencem a tipos diferentes: 2 é um número inteiro, 42.0
é um número de ponto flutuante e `Hello, World!' é uma string, assim
chamada porque as letras que contém estão em uma sequência em cadeia.

Se não tiver certeza sobre qual é o tipo de certo valor, o interpretador
pode dizer isso a você:
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{type}\NormalTok{(}\FloatTok{42.0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{type}\NormalTok{(}\StringTok{\textquotesingle{}Hello World\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Nesses resultados, a palavra ``class'' (classe) é usada no sentido de
categoria; um tipo é uma categoria de valores. Como se poderia esperar,
números inteiros pertencem ao tipo \emph{int}, strings pertencem ao tipo
\emph{str} e os números de ponto flutuante pertencem ao tipo
\emph{float}.

E valores como `2' e `42.0'? Parecem números, mas estão entre aspas como
se fossem strings:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{type}\NormalTok{(}\StringTok{\textquotesingle{}2\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Um dos recursos mais eficientes de uma linguagem de programação é a
capacidade de manipular variáveis. Uma variável é um nome que se refere
a um valor.

\subsection{Instruções de
atribuição}\label{instruuxe7uxf5es-de-atribuiuxe7uxe3o}
...
nova variável chamada message; a segunda dá o número inteiro 17 a n; a
terceira atribui o valor (aproximado) de \(\pi\) a pi. Uma forma comum
de representar variáveis por escrito é colocar o nome com uma flecha
apontando para o seu valor. Este tipo de número é chamado de diagrama de
estado porque mostra o estado no qual cada uma das variáveis está (pense
nele como o estado de espírito da variável).

\subsection{Leitura de erros}\label{leitura-de-erros}

Bla bla bla

\subsection{Nomes de variáveis}\label{nomes-de-variuxe1veis}
...
números, mas não podem começar com um número. É legal usar letras
maiúsculas, mas a convenção é usar apenas letras minúsculas para nomes
de variáveis. O caractere de sublinhar (\_) pode aparecer em um nome.
Muitas vezes é usado em nomes com várias palavras, como
\emph{your\_name} ou \emph{airspeed\_of\_unladen\_swallow}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{more}\OperatorTok{@} \OperatorTok{=} \DecValTok{1000000}
\end{Highlighting}
\end{Shaded}

\begin{Highlighting}
\textcolor{black}{FileNotFoundError: [Errno 2] No such file or directory: '@ = 1000000'}
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{FileNotFoundError}\textcolor{QuartoInternalColor2}{                         Traceback (most recent call last)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[14]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{----> }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{1}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor2}{get\_ipython}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{(}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{)}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{.}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{run\_line\_magic}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{(}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{more}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{,}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{@ = 1000000}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{)}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{File }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{C:\textbackslash{}ProgramData\textbackslash{}anaconda3\textbackslash{}Lib\textbackslash{}site-packages\textbackslash{}IPython\textbackslash{}core\textbackslash{}interactiveshell.py:2504}\textcolor{QuartoInternalColor2}{, in }\textcolor{QuartoInternalColor1}{InteractiveShell.run\_line\_magic}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor5}{(self, magic\_name, line, \_stack\_depth)}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{   2502}\textcolor{QuartoInternalColor2}{     kwargs[}\textcolor{QuartoInternalColor6}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor6}{local\_ns}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor6}{'}\textcolor{QuartoInternalColor2}{] = }\textcolor{QuartoInternalColor7}{self}\textcolor{QuartoInternalColor2}{.get\_local\_scope(stack\_depth)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{   2503}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{with}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{self}\textcolor{QuartoInternalColor2}{.builtin\_trap:}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{-> }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{2504}\textcolor{QuartoInternalColor2}{     result = }\textcolor{QuartoInternalColor2}{fn}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{(}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{*}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{args}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{,}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{*}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{*}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{kwargs}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{)}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{   2506}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor8}{# The code below prevents the output from being displayed}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{   2507}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor8}{# when using magics with decorator @output\_can\_be\_silenced}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{   2508}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor8}{# when the last Python token in the expression is a ';'.}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{   2509}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{if}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{getattr}\textcolor{QuartoInternalColor2}{(fn, magic.MAGIC\_OUTPUT\_CAN\_BE\_SILENCED, }\textcolor{QuartoInternalColor7}{False}\textcolor{QuartoInternalColor2}{):}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{File }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{C:\textbackslash{}ProgramData\textbackslash{}anaconda3\textbackslash{}Lib\textbackslash{}site-packages\textbackslash{}ipykernel\textbackslash{}zmqshell.py:367}\textcolor{QuartoInternalColor2}{, in }\textcolor{QuartoInternalColor1}{KernelMagics.less}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor5}{(self, arg\_s)}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{    365}\textcolor{QuartoInternalColor2}{     cont = }\textcolor{QuartoInternalColor7}{self}\textcolor{QuartoInternalColor2}{.shell.pycolorize(openpy.read\_py\_file(arg\_s, skip\_encoding\_cookie=}\textcolor{QuartoInternalColor7}{False}\textcolor{QuartoInternalColor2}{))}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{    366}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{else}\textcolor{QuartoInternalColor2}{:}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{--> }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{367}\textcolor{QuartoInternalColor2}{     }\textcolor{QuartoInternalColor7}{with}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{open}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{(}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{arg\_s}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{)}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{as}\textcolor{QuartoInternalColor2}{ fid:}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{    368}\textcolor{QuartoInternalColor2}{         cont = fid.read()}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{    369}\textcolor{QuartoInternalColor2}{ page.page(cont)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{FileNotFoundError}\textcolor{QuartoInternalColor2}{: [Errno 2] No such file or directory: '@ = 1000000'}
\end{Highlighting}
...
\textcolor{black}{  }\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[15]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
...
\texttt{76trombones} é ilegal porque começa com um número.
\texttt{more@} é ilegal porque contém um caractere ilegal, o \texttt{@}.
Mas o que há de errado com \texttt{class}?
...
\subsection{Expressões e
instruções}\label{expressuxf5es-e-instruuxe7uxf5es}

Uma expressão é uma combinação de valores, variáveis e operadores. Um
valor por si mesmo é considerado uma expressão, assim como uma variável,
portanto as expressões seguintes são todas legais:
...
Quando você digita uma expressão no prompt, o interpretador a avalia, ou
seja, ele encontra o valor da expressão. Neste exemplo, o n tem o valor
17 e n + 25 tem o valor 42.

Uma instrução é uma unidade de código que tem um efeito, como criar uma
variável ou exibir um valor.
...
A primeira linha é uma instrução de atribuição que dá um valor a n.~A
segunda linha é uma instrução de exibição que exibe o valor de n.~Quando
você digita uma instrução, o interpretador a executa, o que significa
que ele faz o que a instrução diz. Em geral, instruções não têm valores.

\subsection{Pacotes de terceiros e funções
não-nativas}\label{pacotes-de-terceiros-e-funuxe7uxf5es-nuxe3o-nativas}

\subsubsection{Como instalar}\label{como-instalar-1}
...
\subsubsection{Como importar}\label{como-importar}
...
\item
  ...
  \item
    ...
    seu passo médio (tempo por milha em minutos e segundos)? Qual é a
    sua velocidade média em milhas por hora?
  \end{enumerate}
\end{enumerate}
...
Por mais que se conheça a sintaxe e estrutura de uma linguagem de
programação, construir um código eficiente pode não ser das tarefas mais
fáceis. Além disso, traduzir um código de uma determinada linguagem para
outra linguagem pode ser igualmente difícil, mesmo com sintaxes
parecidas.

Utilizar a Inteligência Artificial para fazer isso pela gente pode nos
economizar um tempo valioso. Antes de entender a aplicabilidade dos
modelos de inteligência artificial no mundo da programação, é preciso
primeiro definir inteligência artificial e, principalmente, sua vertente
generativa.
...
\includegraphics[width=7.29167in,height=\textheight,keepaspectratio]{images/IA.jpg}

}

\caption{\label{fig-meme2}Meme 2}

\end{figure}%

\subsection{Pré-requisitos}\label{pruxe9-requisitos-2}

Embora as funções e estruturas necessárias para a realização de
\emph{loops} no Python sejam nativas na linguagem, para algumas das
aplicações utilizaremos um dos pacotes externos que nos permitem
...
projetados para a criação de algo novo, que não estava nos dados
originais. Em essência, os sistema de IAG são sistemas de IA com um
escopo mais restrito e bem definido.
...
dos \emph{Large Language Models} (LLM), que compreendem e geram conteúdo
...
o modelo tivesse por objetivo ``prever'' o conteúdo textual que melhor
se encaixa ao conteúdo fornecido no prompt inicial.

\begin{tcolorbox}[enhanced jigsaw, titlerule=0mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Curiosidade}, coltitle=black, breakable, bottomrule=.15mm, opacitybacktitle=0.6, colback=white, left=2mm, bottomtitle=1mm, colframe=quarto-callout-note-color-frame, arc=.35mm, rightrule=.15mm, opacityback=0, toprule=.15mm, leftrule=.75mm]
...
versões mais recentes. A OpenAI, empresa por trás do ChatGPT, oferece
alguns planos pagos que dão direito a uso ilimitado dessas novas versões
e outras funcionalidades.

Colar imagens

Nem sempre as ferramentas de inteligência artificial, como o ChatGPT,
são indicadas. Questões como acurácia, verificabilidade e uso de dados
sensíveis são relevantes na hora de definir se o ChatGPT é a ferramenta
mais indicada para uma aplicação específica.
...
estruturar uma mensagem de entrada de forma a auxiliar a compreensão dos
modelos de linguagem e ampliar a assertividade e eficiência das
respostas. Apesar de ser uma área em expansão, que conta com várias
estratégias para a construção de prompts altamente personalizados para
cada aplicação, são duas as características essenciais que devem estar
presentes em todo prompt: clareza e especificidade.
...
\subsection{Alguns exemplos}\label{alguns-exemplos}

\subsection{Integração com ferramentas e APIs
externas}\label{integrauxe7uxe3o-com-ferramentas-e-apis-externas}

Nem sempre precisamos de um browser para utilizar o ChatGPT. O acesso às
diferentes versões pode ser realizado através de uma Interface de
Programação de Aplicações, ou API em inglês. Uma API é um conjunto de
regras e protocolos que configuram uma forma simplificada de comunicação
entre diferentes softwares. É possível, por exemplo, acessar as
funcionalidades da ferramenta dentro de um código da linguagem de
programação Python.

No caso do ChatGPT, para acessar a API é preciso se registrar nesse
\href{https://platform.openai.com/}{link} e garantir uma chave de
acesso. A OpenAI já forneceu limites gratuitos para teste da API. Por
conta de sobrecarga dos servidores e uso inadequado, hoje é preciso
pagar pelo uso da API.

Usuários que assinam a versão paga do ChatGPT não têm acesso direto à
API. Para utilizar a API, o usuário cadastra um cartão de crédito e, ao
final do mês, é cobrado pelo número e complexidade das demandas. É
possível configurar limites de uso de modo a evitar gastos maiores do
que o esperado. O preço é diferente para modelos diferentes.
https://openai.com/api/pricing/

Exemplo

\section{StackOverflow}\label{stackoverflow}

\href{https://stackoverflow.com/}{Stack Overflow} é um site de perguntas
e respostas, um fórum, para programadores profissionais e entusiastas. É
o site principal da Stack Exchange Network. Foi criado em 2008 por Jeff
Atwood e Joel Spolsky.Ele apresenta perguntas e respostas sobre uma
ampla gama de tópicos em programação de computadores. Ele foi criado
para ser uma alternativa mais aberta aos sites anteriores de perguntas e
respostas, como o Experts-Exchange. O Stack Overflow foi vendido para a
Prosus, um conglomerado de internet para consumidores com sede na
Holanda, em 2 de junho de 2021 por US\$ 1,8 bilhão.

O site serve como uma plataforma para os usuários fazerem e responderem
perguntas e, por meio de associação e participação ativa, votarem em
perguntas e respostas semelhantes ao Reddit e editarem perguntas e
respostas de maneira semelhante a um wiki. Os usuários do Stack Overflow
podem ganhar pontos de reputação e ``emblemas''; por exemplo, uma pessoa
recebe 10 pontos de reputação por receber um voto ``para cima'' em uma
pergunta ou resposta a uma pergunta, e pode receber medalhas por suas
valiosas contribuições, o que representa uma gamificação do tradicional
Q\&A local na internet. Os usuários desbloqueiam novos privilégios com
um aumento na reputação, como a capacidade de votar, comentar e até
editar as postagens de outras pessoas.

Em março de 2021, o Stack Overflow tinha mais de 14 milhões de usuários
registrados e recebeu mais de 21 milhões de perguntas e 31 milhões de
respostas. Esse fórum, juntamente com outros sites de perguntas e
respostas de programação semelhantes, substituíram globalmente
principalmente os livros de programação para referência de programação
do dia-a-dia nos anos 2000, e hoje são uma parte importante da
programação de computadores. Vocês lembram do nosso gráfico de
participação de algumas das principais linguagens no total de perguntas
do StackOverflow? Qual linguagem está em 1º lugar desde o fim de 2018?

Mas pera lá, o que significa esse monte de adjetivo? (fontes:
\href{https://computersciencewiki.org/}{computersciencewiki.org},
\href{https://www.geeksforgeeks.org/}{geeksforgeeks.org},
\href{https://www.freecodecamp.org}{freecodecamp.org})
...
Nesse capítulo você utilizará essencialmente funções nativas do Python.
No entanto, para trabalhar com padrões textuais de forma mais profunda
utilizaremos pacotes externos:
...
Strings não são como números inteiros ou de ponto flutuate. No Python,
uma string nada mais é do que uma sequência ordenada de caracteres
unicode. Eles são delimitados sempre por aspas (simplas ou duplas).
Relembrando nosso primeiro ``programa'' da aula passada podemos atribuir
a uma variável a string \emph{Hello, World} usando aspas.
...
Note que podemos também usar uma sequência de três aspas duplas e
escrever strings que percorrem várias linhas.
...
aritméticas (em geral) não são permitidas, mas outras operações, como
\emph{slicing}, o são. É possível acessar um caracter específico da
sequência utilizando a posição desse caractere, utilizando o que
chamamos de \textbf{índice}. No caso do string \emph{`Hello, World'},
para acessar a segunda letra podemos utilizar colchetes após o string e
dentro dele o índice referente à posição do `e' no string:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1[}\DecValTok{2}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
particularidade da sintaxe do Python: \textbf{em se tratando de índices,
o Python sempre começa a contagem em 0!}. Dessa forma, para acessar o
primeiro caractere de \emph{`Hello, World'} devemos pedir
\texttt{str1{[}0{]}}, para acessar o segundo é preciso pedir
\texttt{str1{[}1{]}} e assim por diante.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1[}\DecValTok{1}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1[}\OperatorTok{{-}}\DecValTok{11}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
É possível que o índice seja uma expressão, mas deve sempre ser um valor
inteiro.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n}\OperatorTok{=}\DecValTok{0}
\NormalTok{str1[n}\OperatorTok{+}\DecValTok{1}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1[}\FloatTok{1.5}\NormalTok{]}
\end{Highlighting}
\end{Shaded}

\begin{Highlighting}
\textcolor{black}{TypeError: string indices must be integers, not 'float'}
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
...
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[10]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{----> }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{1}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor2}{str1}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{[}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{1.5}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor2}{]}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{TypeError}\textcolor{QuartoInternalColor2}{: string indices must be integers, not 'float'}
\end{Highlighting}

Para acessar uma \textbf{fatia} (\emph{slice}) do string podemos
utilizar um intervalo de índices (inclusive o índice inicial e exclusive
o índice final). Por exemplo,

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1[}\DecValTok{0}\NormalTok{:}\DecValTok{5}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
Além de ser possível calcular o número de caracteres em uma sequência
string utilizando a função integrada \texttt{len()}, podemos
``concatenar'' ou ``somar'' strings utilizando apenas o sinal de +

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{len}\NormalTok{(str1)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1 }\OperatorTok{+} \StringTok{\textquotesingle{} {-}{-}{-} \textquotesingle{}} \OperatorTok{+}\NormalTok{ str2}
\end{Highlighting}
\end{Shaded}
...
Note, porém, que strings são \textbf{imutáveis}, de modo que caso queira
...
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{TypeError}\textcolor{QuartoInternalColor2}{                                 Traceback (most recent call last)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[14]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 2}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{      1}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor8}{# Tentemos substituir o 'e' do str1 por 'a'}\textcolor{QuartoInternalColor2}{}
...
específico. Dentre os principais métodos aplicáveis a strings e suas
funcionalidades podemos citar:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{str\_example.upper()}: devolve a string `str\_example' toda em
  letras maiúsculas.
\item
  \textbf{str\_example.lower()}: devolve a string `str\_example' toda em
  letras minúsculas.
\item
  \textbf{str\_example.strip()}: devolve a string `str\_example'
  retirando possíveis espaços em branco no início e no fim da string.
\item
  \textbf{str\_example.startswith(`xyz')}: testa se a string
  `str\_example' começa com a string `xyz'.
\item
  \textbf{str\_example.endswith(`xyz')}: testa se a string
  `str\_example' termina com a string `xyz'.
\item
  \textbf{str\_example.find(`xyz')}: procura a string `xyz' dentro de
  `str\_example' e retorna o primeiro índice onde `xyz' começa ou
  retorna -1 se nada for encontrado.
\item
  \textbf{str\_example.replace(`old',`new')}: retorna uma string nova
  onde todas as ocorrências de `old' encontradas em `str\_example' serão
...
\BuiltInTok{print}\NormalTok{(str1.upper())}
\BuiltInTok{print}\NormalTok{(str1.lower())}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(str1.strip())}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(str1.startswith(}\StringTok{\textquotesingle{} \textquotesingle{}}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(str1.endswith(}\StringTok{\textquotesingle{}d\textquotesingle{}}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(str1.find(}\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(str1.replace(}\StringTok{\textquotesingle{}Hello\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}World\textquotesingle{}}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\subsection{Formatação e a instrução
print}\label{formatauxe7uxe3o-e-a-instruuxe7uxe3o-print}

Um método sobre o qual não falamos, mas que é bastante interessante
quando queremos, por exemplo, printar bonitinho o resultado de
determinada operação é \texttt{s.format()}. Com esse método podemos
converter uma variável numérica para uma formato específico e printá-la
dentro de um string maior. Imagine, por exemplo, que estejamos
interessados em printar o valor de \(\pi\) arredondado para 2 casas
decimais apenas dentro de um string que diz isso. Podemos implementar
isso da seguinte forma

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pi }\OperatorTok{=} \FloatTok{3.1415926535}
\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}O valor de pi arredondado para 2 casas decimais é }\SpecialCharTok{\{:.2f\}}\StringTok{. Interessante, não?\textquotesingle{}}\NormalTok{.}\BuiltInTok{format}\NormalTok{(pi))}
\end{Highlighting}
\end{Shaded}
...
Podemos também usar mais de um número dentro do mesmo método format. Por
exemplo,

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}O valor de pi arredondado para 2 casas decimais é }\SpecialCharTok{\{:.2f\}}\StringTok{. Com 4 casas decimais, no entanto, o número é um pouco maior e igual a }\SpecialCharTok{\{:.4f\}}\StringTok{. Interessante, não?\textquotesingle{}}\NormalTok{.}\BuiltInTok{format}\NormalTok{(pi,pi))}
\end{Highlighting}
\end{Shaded}
...
\subsection{Introdução a expressões
...
podemos utilizar as famosas \textbf{Expressões Regulares}, também
...
dos chamados \textbf{metacaracteres}, caracteres com funções especiais,
que, agrupados entre si e em conjunto com caracteres literais, formam
uma sequência, uma expressão. Essa expressão é interpretada como uma
regra que indicará sucesso se uma entrada de dados qualquer casar com
essa regra, ou seja, obedecer exatamente a todas as suas condições.
...
um mundo repleto de dados não-estruturados, como tweets e notícias.
\textbf{O potencial de uso é gigante!} Mas como dissemos, o mundo de
\emph{regex} é muito grande e existem cursos e livros que se dedicam
integralmente a estudar essa linguagem de padrões textuais. Uma boa
referência introdutória é o livro
...
\textbf{Se eu pudesse dar um conselho para o meu eu de 15 anos atrás
seria: beba água e estude expressões regulares.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lista2 }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}Danilo Souza\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}Claudio Lucinda\textquotesingle{}}\NormalTok{]}
\NormalTok{lista3 }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}Turma 2024201\textquotesingle{}}\NormalTok{,}\FloatTok{46.0}\NormalTok{,}\StringTok{\textquotesingle{}Turmas 2024202\textquotesingle{}}\NormalTok{,}\DecValTok{49}\NormalTok{,}\StringTok{\textquotesingle{}Turmas 2024221\textquotesingle{}}\NormalTok{,}\DecValTok{81}\NormalTok{]}
...
\subsection{Operações básicas}\label{operauxe7uxf5es-buxe1sicas-1}

A sintaxe para acessar os elementos de uma lista é a mesma que para
acessar os caracteres de uma string: o operador de colchete. A expressão
dentro dos colchetes especifica o índice ou o intervalo de índices.
\textbf{Lembrando que o índice no Python começa em ZERO e não em UM}
...
Diferente das strings, listas são mutáveis. Quando o operador de
colchete aparece do lado esquerdo de uma atribuição, ele identifica o
elemento da lista que será atribuído:
...
\item
  Se tentar ler ou escrever um elemento que não existe, você recebe um
  \texttt{IndexError}.
\item
...
O operador \texttt{in}, que serve ao propósito de testar a existência de
determinado elemento dentro de um objeto específico, também funciona com
listas:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cheeses }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}Cheddar\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Gorgonzola\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Gouda\textquotesingle{}}\NormalTok{]}
\CommentTok{\textquotesingle{}Edam\textquotesingle{}} \KeywordTok{in}\NormalTok{ cheeses}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\textquotesingle{}Brie\textquotesingle{}} \KeywordTok{in}\NormalTok{ cheeses}
\end{Highlighting}
\end{Shaded}
...
``somar'' listas utilizando apenas o sinal de +

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{len}\NormalTok{(cheeses))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lista\_soma }\OperatorTok{=}\NormalTok{ lista2 }\OperatorTok{+}\NormalTok{ cheeses}
\BuiltInTok{print}\NormalTok{(lista\_soma)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Métodos de listas}\label{muxe9todos-de-listas}

As listas também possuem métodos bastante úteis, que nos facilitam a
vida em várias dimensões. Dentre os principais métodos aplicáveis a
listas e suas funcionalidades podemos citar:
...
A maior parte dos métodos de listas são nulos; eles alteram a lista e
retornam None. Se você escrever \texttt{t\ =\ t.sort()} por acidente,
ficará desapontado com o resultado.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}c\textquotesingle{}}\NormalTok{]}
\NormalTok{t.append(}\StringTok{\textquotesingle{}d\textquotesingle{}}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}a\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}b\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}c\textquotesingle{}}\NormalTok{]}
\NormalTok{t2 }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}d\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}e\textquotesingle{}}\NormalTok{]}
\NormalTok{t1.extend(t2)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{s }\OperatorTok{=} \StringTok{\textquotesingle{}spam\textquotesingle{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{s }\OperatorTok{=} \StringTok{\textquotesingle{}pining for the fjords\textquotesingle{}}
\NormalTok{t }\OperatorTok{=}\NormalTok{ s.split(}\StringTok{\textquotesingle{} \textquotesingle{}}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(t)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{s }\OperatorTok{=} \StringTok{\textquotesingle{}spam{-}spam{-}spam\textquotesingle{}}
\NormalTok{t }\OperatorTok{=}\NormalTok{ s.split(}\StringTok{\textquotesingle{}{-}\textquotesingle{}}\NormalTok{)}
\NormalTok{t}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}spam\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}spam\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}spam\textquotesingle{}}\NormalTok{]}
\NormalTok{s }\OperatorTok{=} \StringTok{\textquotesingle{} {-} \textquotesingle{}}\NormalTok{.join(t)}
...
\subsection{Operadores lógicos e variáveis
booleanas}\label{operadores-luxf3gicos-e-variuxe1veis-booleanas}

Agora faremos um pequeno desvio, que será bem útil daqui para frente,
para falar de expressões e variáveis \textbf{booleanas}. Em resumo, uma
expressão booleana é uma expressão que pode ser verdadeira ou falsa e
dessa forma assumir apenas dois valores como resultado: \textbf{True} e
\textbf{False}. Os exemplos seguintes usam o operador de igual no Python
(==) que compara dois operandos e produz True se forem iguais e False se
não forem:
...
\emph{True} e \emph{False} são valores especiais que pertencem ao tipo
\emph{bool}; não são strings. Além disso, é possível fazer operações
aritméticas com variáveis do tipo \emph{bool} já que o Python entende
\emph{True} como sendo equivalente ao número 1 e \emph{False} como sendo
equivalente ao número 0.

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{type}\NormalTok{(}\VariableTok{True}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(}\BuiltInTok{type}\NormalTok{(}\VariableTok{False}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\BuiltInTok{print}\NormalTok{(}\VariableTok{True} \OperatorTok{+} \VariableTok{False}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\VariableTok{False} \OperatorTok{+} \VariableTok{False}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
O operador == é um dos operadores relacionais dentro do Python, os
outros são:
...
Embora essas operações provavelmente sejam familiares para você, os
símbolos do Python são diferentes dos símbolos matemáticos. Um erro
comum é usar apenas um sinal de igual (=) em vez de um sinal duplo (==).
Lembre-se de que = é um operador de atribuição e == é um operador
relacional. Não existe =\textgreater{} ou =\textless.

Há três operadores lógicos: \texttt{and}, \texttt{or} e \texttt{not}. A
semântica (significado) destes operadores é semelhante ao seu
significado em inglês. Por exemplo,
\texttt{x\textgreater{}=0\ and\ x\textless{}=10} só é verdade se x for
maior que 0 \textbf{e} menor que 10.
...
\subsection{Execução condicional}\label{execuuxe7uxe3o-condicional}
...
simples é a instrução \texttt{if}:
...
verdadeira, a instrução indentada é executada. Se não, nada acontece.
\emph{Aqui vale mais um adendo}: uma característica muito importante da
sintaxe do Python é justamente a \textbf{identação}. Diferentemente de
outras linguagens de programação, a identação exerce papel importante
aqui, já que é através dela que se determina onde se inicia e onde
termina um bloco de código, que pode ser uma expressão condicional ou
mesmo uma função, como veremos mais a frente. Além de economizar várias
chaves (``\{'' e ``\}'') e vários ``end'', a identação exerce um papel
estético importante ao permitir uma melhor visualização do código como
um todo.

Não há limite para o número de instruções que podem aparecer no corpo de
uma instrução if, mas deve haver pelo menos uma. Ocasionalmente, é útil
ter um corpo sem instruções (normalmente como um espaço reservado para
código que ainda não foi escrito). Neste caso, você pode usar a
...
\subsection{Execução alternativa}\label{execuuxe7uxe3o-alternativa}

Uma segunda forma da instrução if é a ``execução alternativa'', na qual
há duas possibilidades e a condição determina qual será executada. A
sintaxe pode ser algo assim:
...
\ControlFlowTok{if}\NormalTok{ x }\OperatorTok{\%} \DecValTok{2} \OperatorTok{==} \DecValTok{0}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}x é par\textquotesingle{}}\NormalTok{)}
\ControlFlowTok{else}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}x é ímpar\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Se o resto quando x for dividido por 2 for 0, então sabemos que x é par
e o programa exibe uma mensagem adequada. Se a condição for falsa, o
segundo conjunto de instruções é executado. Como a condição deve ser
verdadeira ou falsa, exatamente uma das alternativas será executada. As
alternativas são chamadas de ramos (branches), porque são ramos no fluxo
da execução.
...
\ControlFlowTok{if} \BuiltInTok{len}\NormalTok{(lista1) }\OperatorTok{\textgreater{}} \DecValTok{2}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}lista1 tem mais de dois elementos\textquotesingle{}}\NormalTok{)}
\ControlFlowTok{else}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}lista1 tem menos de dois elementos\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
ramos. Esta forma de expressar uma operação de computação é uma
...
\texttt{elif} é uma abreviatura de ``else if''. Novamente, exatamente um
ramo será executado. Não há nenhum limite para o número de instruções
elif. Se houver uma cláusula else, ela deve estar no fim, mas não é
preciso haver uma. Cada condição é verificada em ordem. Se a primeira
for falsa, a próxima é verificada, e assim por diante. Se uma delas for
verdadeira, o ramo correspondente é executado e a instrução é encerrada.
Mesmo se mais de uma condição for verdade, só o primeiro ramo verdadeiro
é executado.
...
e podem ser indexados por números inteiros, portanto, nesse sentido, as
tuplas são muito parecidas com as listas. A diferença importante é que
as tuplas são \textbf{imutáveis}, assim como os strings.

Em resumo, uma tupla é uma lista de valores separados por vírgulas e em
geral delimitado por parênteses (lembre que listas são delimitadas por
colchetes):
...
Um único valor entre parênteses não é uma tupla. Para criar uma tupla
com um único elemento, é preciso incluir uma vírgula final após o
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t }\OperatorTok{=} \BuiltInTok{tuple}\NormalTok{()}
\BuiltInTok{print}\NormalTok{(t)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t }\OperatorTok{=} \BuiltInTok{tuple}\NormalTok{(}\StringTok{\textquotesingle{}lupins\textquotesingle{}}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(t)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Operações básicas}\label{operauxe7uxf5es-buxe1sicas-2}

A maior parte dos operadores de lista também funciona em tuplas. O
operador de colchetes indexa um elemento e o operador de fatia seleciona
vários elementos.
...
Entretanto, se tentar alterar um dos elementos da tupla, vai receber um
erro:
...
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{TypeError}\textcolor{QuartoInternalColor2}{                                 Traceback (most recent call last)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[58]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
...
Como tuplas são imutáveis, você não pode alterar os elementos, mas pode
substituir uma tupla por outra.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t }\OperatorTok{=}\NormalTok{ (}\StringTok{\textquotesingle{}A\textquotesingle{}}\NormalTok{,) }\OperatorTok{+}\NormalTok{ t[}\DecValTok{1}\NormalTok{:]}
\BuiltInTok{print}\NormalTok{(t)}
\end{Highlighting}
\end{Shaded}
...
Os operadores relacionais funcionam com tuplas e outras sequências. O
Python começa comparando o primeiro elemento de cada sequência. Se forem
iguais, vai para os próximos elementos, e assim por diante, até que
encontre elementos que sejam diferentes. Os elementos subsequentes não
são considerados (mesmo se forem muito grandes).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{) }\OperatorTok{\textless{}}\NormalTok{ (}\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{2000000}\NormalTok{) }\OperatorTok{\textless{}}\NormalTok{ (}\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Atribuição de tuplas}\label{atribuiuxe7uxe3o-de-tuplas}

Muitas vezes, é útil trocar os valores de duas variáveis. Com a
atribuição convencional, é preciso usar uma variável temporária. Por
...
expressões. Cada valor é atribuído à sua respectiva variável. Todas as
expressões no lado direito são avaliadas antes de todas as atribuições.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a, b }\OperatorTok{=} \DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}
\end{Highlighting}
\end{Shaded}

\begin{Highlighting}
\textcolor{black}{ValueError: too many values to unpack (expected 2)}
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
...
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[64]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{----> }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{1}\textcolor{QuartoInternalColor2}{ a, b = }\textcolor{QuartoInternalColor3}{1}\textcolor{QuartoInternalColor2}{, }\textcolor{QuartoInternalColor3}{2}\textcolor{QuartoInternalColor2}{, }\textcolor{QuartoInternalColor3}{3}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{ValueError}\textcolor{QuartoInternalColor2}{: too many values to unpack (expected 2)}
\end{Highlighting}

De forma geral, o lado direito pode ter qualquer tipo de sequência
(string, lista ou tupla). Por exemplo, para dividir um endereço de email
...
O valor de retorno do \texttt{split()} é uma lista com dois elementos; o
primeiro elemento é atribuído a \texttt{uname}, o segundo a
\texttt{domain}:
...
\subsection{Tuplas como valores de
retorno}\label{tuplas-como-valores-de-retorno}

Falando estritamente, uma função só pode retornar um valor, mas se o
valor for uma tupla, o efeito é o mesmo que retornar valores múltiplos.
Por exemplo, se você quiser dividir dois números inteiros e calcular o
quociente e resto, não é eficiente calcular \texttt{x/y} e depois
\texttt{x\%y}. É melhor calcular ambos ao mesmo tempo. A função
integrada \texttt{divmod} toma dois argumentos e devolve uma tupla de
dois valores: o quociente e o resto da divisão do primeiro termo pelo
segundo termo. Você pode guardar o resultado como uma tupla:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(}\DecValTok{7}\OperatorTok{//}\DecValTok{3}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\DecValTok{7}\OperatorTok{\%}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t }\OperatorTok{=} \BuiltInTok{divmod}\NormalTok{(}\DecValTok{7}\NormalTok{, }\DecValTok{3}\NormalTok{)}
...
\subsection{Operações nativas com
tuplas}\label{operauxe7uxf5es-nativas-com-tuplas}

\texttt{zip} é uma função integrada que recebe duas ou mais sequências e
devolve uma lista de tuplas onde cada tupla contém um elemento de cada
sequência. O nome da função tem a ver com o zíper, que se junta e
encaixa duas carreiras de dentes.
...
\BuiltInTok{zip}\NormalTok{(s,t)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
<zip at 0x2188d948100>
\end{verbatim}

O resultado é um objeto \texttt{zip} que sabe como percorrer os pares. O
uso mais comum de \texttt{zip} é em um loop \texttt{for} (falaremos
sobre loops mais adiante no curso):

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ pair }\KeywordTok{in} \BuiltInTok{zip}\NormalTok{(s, t):}
...
\texttt{zip} para fazer uma lista:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{list}\NormalTok{(}\BuiltInTok{zip}\NormalTok{(s, t))}
\end{Highlighting}
\end{Shaded}
...
O resultado é uma lista de tuplas. Neste exemplo, cada tupla contém um
caractere da string e o elemento correspondente da lista. Se as
sequências não forem do mesmo comprimento, o resultado tem o comprimento
da mais curta:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{list}\NormalTok{(}\BuiltInTok{zip}\NormalTok{(}\StringTok{\textquotesingle{}Anne\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}Elk\textquotesingle{}}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
índices têm que ser números inteiros, em um dicionário eles podem ser de
(quase) qualquer tipo. Um dicionário contém uma coleção de índices, que
se chamam \textbf{chaves} e uma coleção de valores. \textbf{Cada chave é
...
mapa de palavras do inglês ao espanhol, portanto as chaves e os valores
são todos strings.

A função \texttt{dict} cria um novo dicionário sem itens. Como
\texttt{dict} é o nome de uma função integrada, você deve evitar usá-lo
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{eng2sp }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}uno\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}dos\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}three\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}tres\textquotesingle{}}\NormalTok{\}}
\NormalTok{eng2sp}
\end{Highlighting}
\end{Shaded}
...
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[78]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
...
Assim como em strings e listas, a função \texttt{len} também é
compatível com dicionários. Nesse caso ela devolve o número de pares
chave-valor.

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{len}\NormalTok{(eng2sp)}
\end{Highlighting}
\end{Shaded}
...
O operador \texttt{in} funciona em dicionários também. Ele acusa se algo
aparece como chave no dicionário (aparecer como valor não é o
suficiente).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\textquotesingle{}one\textquotesingle{}} \KeywordTok{in}\NormalTok{ eng2sp}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\textquotesingle{}uno\textquotesingle{}} \KeywordTok{in}\NormalTok{ eng2sp}
\end{Highlighting}
\end{Shaded}
...
Para ver se algo aparece como um valor em um dicionário, você pode usar
o método \texttt{values()}, que devolve uma coleção de valores, e então
usar o operador \texttt{in}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{vals }\OperatorTok{=}\NormalTok{ eng2sp.values()}
\CommentTok{\textquotesingle{}uno\textquotesingle{}} \KeywordTok{in}\NormalTok{ vals}
\end{Highlighting}
\end{Shaded}
...
\subsection{Operações básicas}\label{operauxe7uxf5es-buxe1sicas-3}

Podemos adicionar um novo par de chave e valor ao dicionário usando
colchetes, mas caso a chave já exista no dicionário, podemos atualizar o
valor.
...
O valor-chave deve ser um tipo de dados imutável, por essa razão, se
você tentar definir um dicionário com um tipo de dados mutável, o Python
levantará um erro de exceção.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(y)}
\end{Highlighting}
\end{Shaded}
...
Mais uma vez, podemos utilizar a função nativa \texttt{len} para
retornar o número de entradas (pares de valor de chave) em um
dicionário.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{0}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
...
erro de exceção do Python. Para lidar com essa exceção, você pode usar
mais uma vez o operador \texttt{in} que testa se existe uma chave em um
dicionário. Esse operador retorna \emph{True} se o dicionário tiver um
valor armazenado sob a chave fornecida e \emph{Falso} caso contrário.
...
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{KeyError}\textcolor{QuartoInternalColor2}{                                  Traceback (most recent call last)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[88]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 2}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{      1}\textcolor{QuartoInternalColor2}{ y = \{}\textcolor{QuartoInternalColor6}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor6}{one}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor6}{'}\textcolor{QuartoInternalColor2}{: }\textcolor{QuartoInternalColor3}{1}\textcolor{QuartoInternalColor2}{, }\textcolor{QuartoInternalColor6}{'}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor6}{two}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor6}{'}\textcolor{QuartoInternalColor2}{: }\textcolor{QuartoInternalColor3}{2}\textcolor{QuartoInternalColor2}{\}}
...
\subsection{Métodos de dicionários}\label{muxe9todos-de-dicionuxe1rios}
...
\item
  ...
  ``dict\_example'' com todos os pares de valor-chave de um segundo
  dicionário ``x''. Os valores de chaves, que são comuns a ambos os
  dicionários, do segundo dicionário irão se sobrepor aos valores das
...
\item
  ...
  ``dict\_example'' e seus valores associados como uma sequência de
  tuplas.
\item
...
\item
  \textbf{dict\_example.setdefault(x,y)}: é semelhante ao método
  \texttt{get()}: ele retorna o valor associado a uma chave ``x'' se o
  dicionário contiver essa chave, mas caso o dicionário não contenha a
  chave, este método criará um novo elemento no dicionário (par de valor
  de chave), onde o primeiro argumento neste método é a chave, e o
  segundo argumento (``y'') é o valor. O segundo argumento é opcional,
  mas se isso não for incluído, o valor atrelado a essa nova chave será
  \emph{None}.
\end{enumerate}
...
\begin{Highlighting}[]
\NormalTok{x }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{0}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
\NormalTok{y }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}three\textquotesingle{}}\NormalTok{: }\DecValTok{3}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(x)}

\NormalTok{x.update(y)}
\BuiltInTok{print}\NormalTok{(x)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(x.keys())}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{x }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(x.items())}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{y }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(y.get(}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(y.get(}\StringTok{\textquotesingle{}three\textquotesingle{}}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(y.get(}\StringTok{\textquotesingle{}three\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}The key does not exist.\textquotesingle{}}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{y }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}one\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{: }\DecValTok{2}\NormalTok{\}}
\BuiltInTok{print}\NormalTok{(y.setdefault(}\StringTok{\textquotesingle{}three\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}3\textquotesingle{}}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(y.setdefault(}\StringTok{\textquotesingle{}two\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}dos\textquotesingle{}}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(y)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(y.setdefault(}\StringTok{\textquotesingle{}four\textquotesingle{}}\NormalTok{))}
\BuiltInTok{print}\NormalTok{(y)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \texttt{fruit\ =\ {[}\textquotesingle{}pear\textquotesingle{},\ \textquotesingle{}orange\textquotesingle{},\ \textquotesingle{}apple\textquotesingle{},\ \textquotesingle{}grapefruit\textquotesingle{},\ \textquotesingle{}apple\textquotesingle{},\ \textquotesingle{}pear\textquotesingle{}{]}}.
  Use uma função de lista para dizer o índice da primeira ocorrência de
  \texttt{apple}
\item
  Usando a lista do exercício anterior, use uma função de lista para
  retornar o número de veze que \texttt{apple} ocorre.
\item
  Existem duas listas abaixo. Escreva um programa que as converta em um
  dicionário em que o item de \texttt{keys} é a chave e o item de
  \texttt{values} é o valor
...
\CommentTok{\#\#\# Output esperado}
\NormalTok{\{}\StringTok{\textquotesingle{}Ten\textquotesingle{}}\NormalTok{: }\DecValTok{10}\NormalTok{, }\StringTok{\textquotesingle{}Twenty\textquotesingle{}}\NormalTok{: }\DecValTok{20}\NormalTok{, }\StringTok{\textquotesingle{}Thirty\textquotesingle{}}\NormalTok{: }\DecValTok{30}\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
\end{enumerate}
...
\section{\texorpdfstring{Repetição simples e a instrução
\emph{for}}{Repetição simples e a instrução for}}\label{repetiuxe7uxe3o-simples-e-a-instruuxe7uxe3o-for}

Os computadores muitas vezes são usados para automatizar tarefas
repetitivas. A repetição de tarefas idênticas ou semelhantes sem cometer
erros é algo que os computadores fazem muito bem e as pessoas não. Em um
programa de computador, a repetição também é chamada de iteração.
...
dois pontos, e um corpo \textbf{identado} que é executado uma vez por
iteração. O corpo pode conter qualquer número de instruções, mas o
Python só reconhecerá como parte do corpo o código que estiver identado
...
que vai de 0 a 1. Utilizando o objeto \texttt{range()} e a lógica de
listas:
...
momento, de modo que a lista nos serviu apenas para ditar o número de
vezes que gostaríamos de rodar o bloco de instruções abaixo do
cabeçalho, no caso 10. Podemos ir um pouco além.
...
\NormalTok{    u }\OperatorTok{=}\NormalTok{ np.random.uniform()}
    \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}Essa é a iteração \textquotesingle{}}\OperatorTok{+}\BuiltInTok{str}\NormalTok{(i}\OperatorTok{+}\DecValTok{1}\NormalTok{)}\OperatorTok{+}\StringTok{\textquotesingle{} e o número sorteado foi: }\SpecialCharTok{\{:.2f\}}\StringTok{\textquotesingle{}}\NormalTok{.}\BuiltInTok{format}\NormalTok{(u)) }
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Essa é a iteração 1 e o número sorteado foi: 0.54
Essa é a iteração 2 e o número sorteado foi: 0.01
Essa é a iteração 3 e o número sorteado foi: 0.20
Essa é a iteração 4 e o número sorteado foi: 0.13
Essa é a iteração 5 e o número sorteado foi: 0.53
Essa é a iteração 6 e o número sorteado foi: 0.58
Essa é a iteração 7 e o número sorteado foi: 0.66
Essa é a iteração 8 e o número sorteado foi: 0.30
Essa é a iteração 9 e o número sorteado foi: 0.86
Essa é a iteração 10 e o número sorteado foi: 0.57
\end{verbatim}
...
Como dito anterioremente, o loop \texttt{for} funciona com qualquer
sequência ou objeto pelo qual seja possível percorrer. Podemos
trabalhar, inclusive, com strings e listas como nos exemplos abaixo:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1 }\OperatorTok{=} \StringTok{\textquotesingle{}Vou nadaaaa\textquotesingle{}}

\ControlFlowTok{for}\NormalTok{ c }\KeywordTok{in}\NormalTok{ str1:}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lista1 }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}Minha terra tem palmeiras\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}onde canta o sabiá\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}seno A cosseno B\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}seno B cosseno A\textquotesingle{}}\NormalTok{]}

\ControlFlowTok{for}\NormalTok{ f }\KeywordTok{in}\NormalTok{ lista1:}
...
\subsection{\texorpdfstring{\emph{for} loop e instruções
condicionais}{for loop e instruções condicionais}}\label{for-loop-e-instruuxe7uxf5es-condicionais}
...
todos os elementos da sequência alvo.

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ i }\KeywordTok{in}\NormalTok{ [}\StringTok{\textquotesingle{}foo\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}bar\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}baz\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}qux\textquotesingle{}}\NormalTok{]:}
    
    \ControlFlowTok{if} \StringTok{\textquotesingle{}b\textquotesingle{}} \KeywordTok{in}\NormalTok{ i:}
...
\texttt{{[}\textquotesingle{}foo\textquotesingle{},\textquotesingle{}bar\textquotesingle{},\textquotesingle{}baz\textquotesingle{},\textquotesingle{}qux\textquotesingle{}{]}}
contém a letra `b' mas o primeiro não. A instrução \texttt{if},
portanto, retorna \texttt{False} na primeira iteração e \texttt{True} na
segunda. No momento em que ela retorna \texttt{True} o código identado
\texttt{break} é executado e o loop para aí! Dessa forma, o único termo
que a instrução print vai imprimir será o primeiro da lista, isto é,
`foo'.

\subsection{\texorpdfstring{Usando \emph{enumerate} e
\emph{zip}}{Usando enumerate e zip}}\label{usando-enumerate-e-zip}

Às vezes pode ser útil, dentro de um loop \texttt{for}, ter uma variável
que muda em cada iteração do loop que possibilite fazer um controle mais
direto sobre qual iteração está sendo executada em determinado momento.
Em vez de criar e incrementar uma variável você mesmo, você pode usar a
função nativa \texttt{enumerate()} para obter um contador e o valor do
objeto iterável ao mesmo tempo! Para deixar claro como funciona a
sintaxe nesse caso, vamos trabalhar em cima de uma lista de strings.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{str1 }\OperatorTok{=} \StringTok{\textquotesingle{}O Tata é foota, o Tata é foota\textquotesingle{}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lista2 }\OperatorTok{=} \BuiltInTok{list}\NormalTok{(}\BuiltInTok{range}\NormalTok{(}\DecValTok{0}\NormalTok{,}\BuiltInTok{len}\NormalTok{(lista1)))}
\BuiltInTok{print}\NormalTok{(lista2)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{len}\NormalTok{(lista1) }\OperatorTok{==} \BuiltInTok{len}\NormalTok{(lista2)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ count,value }\KeywordTok{in} \BuiltInTok{zip}\NormalTok{(lista2, lista1):}
...
Embora tenhamos utilizado a função \texttt{zip()} para chegar ao mesmo
resultado de \texttt{enumerate()}, seu escopo de ação é muito mais geral
já que nos permite percorrer dois objetos distintos ao mesmo tempo,
sejam quais forem esses objetos. A única obrigatoriedade é que esses
objetos tenham o mesmo tamanho, nada mais!

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{professores }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}Danilo Souza\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}Danilo Souza\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}Claudio Lucinda\textquotesingle{}}\NormalTok{]}
\NormalTok{turmas }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}2024201\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}2024202\textquotesingle{}}\NormalTok{,}\StringTok{\textquotesingle{}2024221\textquotesingle{}}\NormalTok{]}
...
\subsection{\texorpdfstring{\emph{For} e \emph{list
comprehensions}}{For e list comprehensions}}\label{for-e-list-comprehensions}

\emph{List comprehensions} é uma das várias formas que o Python nos
oferece para de criar, alterar e manipular listas. Sua sintaxe é
bastante concisa e nos permite olhar para um exerício de iteração
através de algo parecido com uma fórmula.

Imagine que estejamos interessados em criar uma lista que contenha a
parte inteira da raiz quadrada de todo número de uma outra lista
original. Usualmente faríamos isso utilizando um loop \texttt{for}:
...
de uma única linha de código. Mas porque aprender as duas formas? Será
que alguma delas é mais eficiente? Vamos fazer um teste para esse
exercicío da raiz quadrada, porém para uma lista maior. Utilizaremos
também a função \texttt{time} do pacote \texttt{time} para calcular o
tempo utilizado por cada um dos métodos.
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}Tempo necessário para a realização dos cálculos utilizando list comprehensions: }\SpecialCharTok{\{:.4f\}}\StringTok{ segundos\textquotesingle{}}\NormalTok{.}\BuiltInTok{format}\NormalTok{(end\_comp }\OperatorTok{{-}}\NormalTok{ start\_comp))}
\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}Tempo necessário para a realização dos cálculos utilizando loop: }\SpecialCharTok{\{:.4f\}}\StringTok{ segundos\textquotesingle{}}\NormalTok{.}\BuiltInTok{format}\NormalTok{(end\_loop }\OperatorTok{{-}}\NormalTok{ start\_loop))}
\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{A abordagem de loop demorou }\SpecialCharTok{\{:.1\%\}}\StringTok{ mais tempo! Dê uma chance para list comprehensions ;)\textquotesingle{}}\NormalTok{.}\BuiltInTok{format}\NormalTok{(ratio }\OperatorTok{{-}} \DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Tempo necessário para a realização dos cálculos utilizando list comprehensions: 0.0913 segundos
Tempo necessário para a realização dos cálculos utilizando loop: 0.1244 segundos

A abordagem de loop demorou 36.3% mais tempo! Dê uma chance para list comprehensions ;)
\end{verbatim}

\section{Reatribuição e atualização de
variáveis}\label{reatribuiuxe7uxe3o-e-atualizauxe7uxe3o-de-variuxe1veis}

Pode ser que você já tenha descoberto que é permitido fazer mais de uma
atribuição para a mesma variável. Uma nova atribuição faz uma variável
existente referir-se a um novo valor (e deixar de referir-se ao valor
anterior).
...
A primeira vez que exibimos x, seu valor é 5. Na segunda vez, seu valor
é 7.

Neste ponto quero tratar de uma fonte comum de confusão. Como o Python
usa o sinal de igual (=) para atribuição, é tentador interpretar uma
afirmação como \texttt{a\ =\ b} como uma proposição matemática de
igualdade, isto é, a declaração de que a e b são iguais. Mas esta é uma
interpretação equivocada.

Em primeiro lugar, a igualdade é uma relação simétrica e a atribuição
não é. Por exemplo, na matemática, se \texttt{a=7} então \texttt{7=a}.
Mas no Python, a instrução \texttt{a\ =\ 7} é legal e \texttt{7\ =\ a}
não é. Além disso, na matemática, uma proposição de igualdade é
verdadeira ou falsa para sempre. Se \texttt{a=b} agora, então \texttt{a}
sempre será igual a \texttt{b}. No Python, uma instrução de atribuição
pode tornar duas variáveis iguais, mas elas não precisam se manter
assim:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a }\OperatorTok{=} \DecValTok{5}
\NormalTok{b }\OperatorTok{=}\NormalTok{ a }\CommentTok{\# a e b agora são iguais}

\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}Valor de a: \textquotesingle{}}\OperatorTok{+}\BuiltInTok{str}\NormalTok{(a)}\OperatorTok{+}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{Valor de b: \textquotesingle{}}\OperatorTok{+}\BuiltInTok{str}\NormalTok{(b))}

\NormalTok{a }\OperatorTok{=} \DecValTok{3} \CommentTok{\# a e b não são mais iguais}

\BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{Valor de a: \textquotesingle{}}\OperatorTok{+}\BuiltInTok{str}\NormalTok{(a)}\OperatorTok{+}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{Valor de b: \textquotesingle{}}\OperatorTok{+}\BuiltInTok{str}\NormalTok{(b))}
\end{Highlighting}
\end{Shaded}
...
Quando redefinimos o valor de \texttt{a} para 3, não mudamos o valor de
\texttt{b}, de modo que as variáveis deixam de ser iguais. A
reatribuição de variáveis muitas vezes é útil, mas você deve usá-la com
prudência. Se os valores das variáveis mudarem frequentemente, isso pode
dificultar a leitura e depuração do código.

\section{\texorpdfstring{Repetição condicional e a instrução
\emph{while}}{Repetição condicional e a instrução while}}\label{repetiuxe7uxe3o-condicional-e-a-instruuxe7uxe3o-while}

Em programação existem dois tipos de iteração, indefinidas e definidas:

\begin{itemize}
\item
  Com \textbf{iteração definida}, o número de vezes que o bloco
  designado será executado é especificado explicitamente no momento em
  que o loop é iniciado. É o exemplo do \texttt{for} loop com o qual
  trabalhamos anteriormente.
\item
  Com \textbf{iteração indefinida}, o número de vezes que o loop é
...
Para interação indefinida, a construção típica do Python é o
\texttt{while}. A forma mais simples desse loop \texttt{while} é a
seguinte:

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{while} \OperatorTok{\textless{}}\NormalTok{expr}\OperatorTok{\textgreater{}}\NormalTok{:}
    \OperatorTok{\textless{}}\NormalTok{instruções}\OperatorTok{\textgreater{}}
\end{Highlighting}
\end{Shaded}

Em que \texttt{instruções} representam uma ou mais linhas de código a
serem executadas. Da mesma forma que nas outras estruturas, a
\textbf{indentação} determina quais linhas vão ser executadas.

A expressão de controle, \texttt{\textless{}expr\textgreater{}},
normalmente envolve uma ou mais variáveis que são inicializadas antes de
iniciar o loop e depois modificadas em algum lugar no corpo do loop.
\textbf{Você precisa dessa expressão bem pensada, porque é ela que vai
determinar o momento em que o loop para}

Quando um loop \texttt{while} é encontrado,
\texttt{\textless{}expr\textgreater{}}é avaliado primeiro no contexto
booleano. Se for verdadeiro, o corpo do loop é executado. Em seguida
\texttt{\textless{}expr\textgreater{}}, é verificado novamente e, se
ainda for verdadeiro, o corpo é executado novamente. Isso continua até
\texttt{\textless{}expr\textgreater{}} se tornar falso, momento em que a
execução do programa prossegue para a primeira instrução além do corpo
do loop. \textbf{Dentro de \texttt{\textless{}instruções\textgreater{}}
você precisa ter algo que modifique algum elemento de
\texttt{\textless{}expr\textgreater{}}. Senão ou o loop não vai rodar ou
vai rodar para sempre.} Uma das formas de interromper \textbf{à força} a
execução de um loop é por meio das teclas \texttt{Ctrl+C}.
...
O valor inicial de \texttt{n} é 5. A expressão no cabeçalho da instrução
\texttt{while} na linha 2 é \texttt{n\ \textgreater{}\ 0}, o que é
\texttt{True}, então o corpo do loop é executado. Dentro do corpo do
loop redefinimos o valor de \texttt{n} para que seja igual ao valor
anterior de \texttt{n} menos 1, e então o novo valor de \texttt{n} é
impresso.

Quando o corpo do loop termina, a execução do programa retorna ao topo
do loop na linha 2 e a expressão é avaliada novamente. Ainda é verdade,
então o corpo é executado novamente e 3 é impresso. Isso continua até
\texttt{n} se tornar 0. Nesse ponto, quando a expressão é testada, ela é
falsa (0 não é maior do que 0) e o loop termina. A execução é retomada
na primeira instrução após o corpo do loop, no caso a instrução print.
Observe que a expressão de controle do loop \texttt{while} é testada
primeiro, antes que qualquer outra coisa aconteça.

A seguir temos um loop while que usa uma lista:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a }\OperatorTok{=}\NormalTok{ [}\StringTok{\textquotesingle{}foo\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}bar\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}baz\textquotesingle{}}\NormalTok{]}
...
Quando uma lista é avaliada em contexto booleano, ela é verdadeira se
tiver elementos nela e falsa se estiver vazia. Neste exemplo, \texttt{a}
é verdadeiro desde que tenha elementos nele. Quando todos os itens forem
removidos com o método \texttt{.pop()} e a lista estiver vazia,
\texttt{a} será falsa e o loop terminará.

\subsection{Formas de Interromper um
Loop}\label{formas-de-interromper-um-loop}

Em cada exemplo que você viu até agora, todo o corpo do loop
\texttt{while} é executado em cada iteração. Python fornece duas
palavras-chave que encerram uma iteração de loop prematuramente:

\begin{itemize}
\item
  A instrução Python \texttt{break} imediatamente encerra um loop
  inteiramente. Já vimos como ela funciona no caso do \texttt{for} loop
  e instruções condicionais.
\item
  A instrução \texttt{continue}, por outro lado, encerra imediatamente a
  iteração do loop atual. A execução salta para o topo do loop e a
  expressão de controle é reavaliada para determinar se o loop será
  executado novamente ou terminará.
\end{itemize}

Um exemplo de \texttt{break}:
...
Obs: esse exercício é o mais complexo que fizemos até agora. Nele
utilizamos grande parte dos conceitos que estudamos até aqui nesse
curso, desde listas a loops e também a importação de bibliotecas de
comandos. Você pode pular essa parte por enquanto, mas aconselho que aos
poucos você tente replicá-lo, de forma a entender todas suas etapas com
o tempo.

O \textbf{Teorema Central do Limite} é um resultado super importante em
estatística e com aplicações nas mais diversas áreas. Em sua formulação
mais simples, o teorema diz que:

A distribuição da média amostral da variável aleatória \(X\) aproxima-se
cada vez mais de uma distribuição normal conforme aumenta o tamanho da
amostra, independentemente da distribuição original de X.

Suponha que estejamos amostrando de uma variável aleatória X com média
finita e igual a \(\mu\) e desvio padrão finito e igual a \(\sigma\).
Então média e desvio padrão da distribuição amostral são representados
por:
...
Vamos apresentar uma implementação desse Teorema usando Python, em
várias partes. A primeira parte mostra como tirar 10 números aleatórios
entre -40 e 40, usando o \texttt{NumPy}. Mais uma vez utilizaremos a
função \texttt{uniform()} já que elas sorteia números reais com igual
probabilidade dentro de um intervalo especificado.
...
\begin{verbatim}
[ 18.12450652   0.25254856  27.22361833 -10.92810639  31.40221884
  -3.06088944 -13.10505989  28.38830602 -30.39800288  23.62735138]
\end{verbatim}
...
\begin{verbatim}
[-21.76549415 -38.23148322  -6.21483268  37.50206353   4.58799378
 -13.50546766  -2.02122137 -28.40389673 -16.55020062  35.51932393]
\end{verbatim}

Para tornar o nosso exemplo mais previsível, vamos usar uma função
dentro do \texttt{NumPy} que faz com o que o sorteio dos números parta
do mesmo lugar e, portanto, o resultado seja o mesmo.
...
uma amostra não tenha a média igual à média populacional, a
\textbf{média das médias} vai convergindo para zero à medida em que
extraímos novas amostras e calculamos novas médias.

Agora vou fazer isso em um loop, sorteando 50 números aleatórios por
amostra e primeiro com 10 amostras:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# number of sample}
...
\BuiltInTok{print}\NormalTok{(means)}
\NormalTok{np.mean(means)}
\end{Highlighting}
\end{Shaded}
...
Vamos deixar essa tentativa de aproximar a média das médias de \(\mu\)
mais automatizada:
...
Note que até aqui a gente só mostrou que a \textbf{média das médias
converge para a média populacional}. O Teorema, no entanto, é mais
completo do que isso. Ele diz que a distribuição da média amostral é uma
Normal e, além disso, possui aquelas características em relação à
distribuição original. Ainda precisamos de mais algumas aulas para ir
...
No contexto das linguagens de programação, uma função é uma sequência
nomeada de instruções, que executa algum tipo de operação específica mas
não necessariamente numérica, como é o caso das funções matemáticas. A
ideia essencial por trás de uma função é a de juntar algumas tarefas
comuns ou repetidas e criar uma função para que, em vez de escrever o
mesmo código várias vezes, possamos chama-la pelo nome e reutilizar o
conjunto de instruções nela contida sempre que necessário.
...
que daremos à função vem logo em seguida. É através desse nome,
\texttt{nome\_da\_funcao}, que chamaremos essa função em outras partes
do nosso código. Entre parênteses definimos os \texttt{argumentos} que a
função recebe para realizar o conjunto de instruções em
\texttt{\textless{}instruções\textgreater{}}.

Note, mais uma vez, que todo o bloco de código que estiver
\textbf{identado} e abaixo da linha de cabeçalho da função fará parte da
função. Assim como nos \emph{loops}, a função termina quando passarmos a
primeira linha de código não-identada.

\section{Nossa primeira função}\label{nossa-primeira-funuxe7uxe3o}

Vamos começar criando uma função simples, que tem por objetivo printar
uma das frases mais conhecidas da história do cinema:
...
você chamar a função \texttt{frase\_cinema}, ela vai fazer a mesma
coisa. Pode até ser que seja o seu objetivo fazer exatamente isso -- é
uma forma de economizar código.

O ponto é que a ideia de função é muito mais poderosa do que
simplesmente uma ``abreviação'' de um monte de linhas de código. A
abstração implícita no conceito de função é poderosa o suficiente para
garantir que a função retorne coisas diferentes caso você altere algum
\textbf{argumento}. Vamos falar disso a seguir.
...
\KeywordTok{def}\NormalTok{ my\_func(name,place):}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Olá }\SpecialCharTok{\{}\NormalTok{name}\SpecialCharTok{\}}\SpecialStringTok{! Você é de }\SpecialCharTok{\{}\NormalTok{place}\SpecialCharTok{\}}\SpecialStringTok{?"}\NormalTok{)}

\NormalTok{my\_func(}\StringTok{"Emily"}\NormalTok{,}\StringTok{"Paris"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
O que acontece se você especificar o \texttt{place} primeiro e depois o
\texttt{name}? Vamos descobrir.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{my\_func(}\StringTok{"Hawaii"}\NormalTok{,}\StringTok{"Robert"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Meio bizarro, não? A razão é que aqui temos os chamados
\textbf{argumentos posicionais}. Ou seja, a função vai assumir que o
primeiro argumento é o \texttt{name} e o segundo é o \texttt{place} não
importa o que tenhamos passado como argumento. Para lidar com isso, a
gente pode atribuir um nome a cada um dos argumentos, ou
\textbf{palavra-chave}, que aí vai ser a palavra-chave, e não a posição,
que vai determinar qual o valor atribuído a cada argumento.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{my\_func(place}\OperatorTok{=}\StringTok{"Hawaii"}\NormalTok{,name}\OperatorTok{=}\StringTok{"Robert"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Vimos aqui que a posição passou a ser irrelevante porque colocamos os
nomes de cada um dos argumentos. E se quiséssemos dar mais flexibilidade
ainda à função, só especificando um subconjunto dos argumentos? Para que
isso funcione, nós precisamos especificar os \textbf{valores-padrão} dos
argumentos caso eles não sejam fornecidos.
...
\NormalTok{    total }\OperatorTok{=} \BuiltInTok{round}\NormalTok{(total,}\DecValTok{2}\NormalTok{)}
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Please pay $}\SpecialCharTok{\{}\NormalTok{total}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Nesse caso, o argumento \texttt{bill\_amount} é obrigatório. Por outro
lado, o argumento \texttt{tip\_perc} vai assumir o valor de 10 toda vez
que ele não for explicitamente fornecido na chamada da função.

\subsection{Argumentos arbitrários}\label{argumentos-arbitruxe1rios}

Vamos começar fazendo algumas perguntas: * E se não soubermos o número
exato de argumentos de antemão? * Podemos criar funções que funcionem
com um número variável de argumentos?

A resposta é \emph{sim}! E vamos criar essa função imediatamente. Vamos
criar uma função simples \texttt{my\_var\_sum()} que retorna a soma de
todos os números passados como argumento. No entanto, o número de
argumentos pode ser potencialmente diferente cada vez que chamamos a
função.
...
\KeywordTok{def}\NormalTok{ my\_var\_sum(}\OperatorTok{*}\NormalTok{args):}
    ...
    \ControlFlowTok{for}\NormalTok{ arg }\KeywordTok{in}\NormalTok{ args:}
        \BuiltInTok{sum} \OperatorTok{+=}\NormalTok{ arg}
        
    \BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"The numbers that you have add up to }\SpecialCharTok{\{}\BuiltInTok{sum}\SpecialCharTok{\}}\SpecialStringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Observe como a definição da função agora tem \texttt{*args} em vez de
apenas o nome do parâmetro. No corpo da função, fazemos um loop em
...
sequência de valores \emph{com nomes}? Não se preocupe, existe uma
possibilidade de fazer isso, usando o \texttt{**kwargs} na definição dos
argumentos. Qual é a diferença entre \texttt{*args} e \texttt{**kwargs}?
A diferença é que você vai passar uma sequência de tamanho arbitrário de
parâmetros, cada um deles nomeado.

O Python vai entender o \texttt{**kwargs} como um dicionário. Cada
elemento passado é um par chave-valor, e dentro da função você tem que
desempacotar o valor da chave. Vamos seguir alguns exemplos.
...
\KeywordTok{def}\NormalTok{ myFun(}\OperatorTok{**}\NormalTok{kwargs):}
    ...
    \ControlFlowTok{for}\NormalTok{ key, value }\KeywordTok{in}\NormalTok{ kwargs.items():}
        \BuiltInTok{print}\NormalTok{(}\StringTok{"}\SpecialCharTok{\%s}\StringTok{ == }\SpecialCharTok{\%s}\StringTok{"} \OperatorTok{\%}\NormalTok{ (key, value))}
...
\texttt{*args}/\texttt{*kwargs})? Sim, como podemos ver no exemplo
seguinte:
...
\KeywordTok{def}\NormalTok{ myFun(arg1, }\OperatorTok{**}\NormalTok{kwargs):}
    ...
    \ControlFlowTok{for}\NormalTok{ key, value }\KeywordTok{in}\NormalTok{ kwargs.items():}
        \BuiltInTok{print}\NormalTok{(arg1}\OperatorTok{+}\StringTok{" }\SpecialCharTok{\%s}\StringTok{ == }\SpecialCharTok{\%s}\StringTok{"} \OperatorTok{\%}\NormalTok{ (key, value))}
...
\subsection{Elementos dentro da função: execução
condicional}\label{elementos-dentro-da-funuxe7uxe3o-execuuxe7uxe3o-condicional}

As funções podem retornar objetos booleanos também, o que pode ser
conveniente para esconder testes complicados dentro de funções. Por
exemplo:
...
\subsection{Aplicação: Teorema Central do Limite - Parte
1}\label{aplicauxe7uxe3o-teorema-central-do-limite---parte-1}
...
intervalo no qual a variável está definida e o número de sorteios que
faremos em cada amostra. Com o que aprendemos até agora podemos definir
a função \texttt{func\_tcl} abaixo:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ func\_tcl(dist}\OperatorTok{=}\VariableTok{None}\NormalTok{,intervalo}\OperatorTok{=}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{),n}\OperatorTok{=}\DecValTok{100}\NormalTok{):}
    
    \ControlFlowTok{if}\NormalTok{ dist }\OperatorTok{==} \VariableTok{None}\NormalTok{:}
        \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}Você esqueceu de carregar uma função que defina a distribuição de X! Volte 2 casas.\textquotesingle{}}\NormalTok{)}
    \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        x }\OperatorTok{=}\NormalTok{ dist(intervalo[}\DecValTok{0}\NormalTok{],intervalo[}\DecValTok{1}\NormalTok{],n)}
        \BuiltInTok{print}\NormalTok{(x)}
\end{Highlighting}
\end{Shaded}

Para todos os 3 argumentos definimos valores \emph{default} que, caso
não sejam alterados a função vai utilizá-los em suas operações. Vamos
rodar a função com todos os valores \emph{default} e ver o que acontece:
...
Nesse caso, a execução condicional \texttt{if} dentro da função acende
já que não passamos nenhuma distribuição como default, apenas o valor
\texttt{None}. Vamos então repetir o que fizemos na aula passada e
utilizar a distribuição uniforme, no intervalo \(\left[-40,40\right]\) e
sorteando apenas 10 números desta distribuição:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{func\_tcl(dist}\OperatorTok{=}\NormalTok{np.random.uniform,intervalo}\OperatorTok{=}\NormalTok{(}\OperatorTok{{-}}\DecValTok{40}\NormalTok{,}\DecValTok{40}\NormalTok{),n}\OperatorTok{=}\DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
[ -9.80286201  28.89601606 -15.28476707 -16.30399747 -18.294223
  28.04406173  22.59099701  36.87747356  33.79447666 -37.99965729]
\end{verbatim}

\subsection{Elementos dentro da função:
iteração}\label{elementos-dentro-da-funuxe7uxe3o-iterauxe7uxe3o}
...
da função, assim como no caso de \texttt{*args}. Vamos olhar para um
outro exemplo em que a função recebe uma lista de strings como argumento
e faz operações em cada um dos elementos, um a um:
...
\subsection{Aplicação: Teorema Central do Limite - Parte
2}\label{aplicauxe7uxe3o-teorema-central-do-limite---parte-2}

Agora podemos dar mais um passo e incorporar o loop dentro da função,
passando como novo argumento da função o número de amostras com o qual
queremos trabalhar:
...
\NormalTok{            means.append(mean\_x)}
        ...
        \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}Essa é a lista de médias:\textquotesingle{}}\NormalTok{)}
        \BuiltInTok{print}\NormalTok{(means)}
        \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{E essa é a média das médias:\textquotesingle{}}\NormalTok{)}
        \BuiltInTok{print}\NormalTok{(}\BuiltInTok{sum}\NormalTok{(means)}\OperatorTok{/}\BuiltInTok{len}\NormalTok{(means))}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# utilizamos a função seed mais uma vez para tornar os resultados previsíveis}
\NormalTok{np.random.seed(}\DecValTok{1}\NormalTok{)}
...
Mas note que o valor da média que nos interessa nesse caso só aparece
printado quando chamamos a função. O que podemos fazer para conseguir
guardá-lo como uma nova variável, por exemplo?

\section{Valor de retorno de uma
função}\label{valor-de-retorno-de-uma-funuxe7uxe3o}

O comando \texttt{return} é usado para a saída de uma função de volta ao
lugar no código de onde ela foi chamada. A sintaxe desse comando é
simples:

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{return} \OperatorTok{\textless{}}\NormalTok{expression\_list}\OperatorTok{\textgreater{}}
\end{Highlighting}
\end{Shaded}

Essa declaração pode ter um comando que é executado e o valor resultante
devolvido. Se esse comando não for anexado ou não tiver nada além do
\texttt{return}, a função é devolvida com o valor de \texttt{None}. Por
exemplo:
...
\BuiltInTok{print}\NormalTok{(my\_func(}\StringTok{"Jane"}\NormalTok{,}\StringTok{"Paris"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Funções que não contenham o comando \texttt{return} sempre retornarão um
valor vazio. Quando queremos utilizar uma função com o objetivo de
realizar algum tipo de operação e \textbf{guardar} o valor dessa
operação, não podemos esquecer do \texttt{return}. Vejamos o exemplo da
função que calcula a raiz quadrada de um número:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ raiz(x):}
    
\NormalTok{    fx }\OperatorTok{=}\NormalTok{ x}\OperatorTok{**}\FloatTok{0.5}

\NormalTok{y }\OperatorTok{=}\NormalTok{ raiz(}\DecValTok{100}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(y)}
\end{Highlighting}
\end{Shaded}
...
Quando você cria uma variável \textbf{dentro} de uma função ela é local,
ou seja, só existe dentro da própria função. Por exemplo:
...
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[23]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
...
\subsection{Aplicação: Teorema Central do Limite - Parte
3}\label{aplicauxe7uxe3o-teorema-central-do-limite---parte-3}
...
\NormalTok{            mean\_of\_means }\OperatorTok{=} \BuiltInTok{sum}\NormalTok{(means)}\OperatorTok{/}\BuiltInTok{len}\NormalTok{(means)}
        ...
        \BuiltInTok{print}\NormalTok{(means)}
        \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{E essa é a média das médias:\textquotesingle{}}\NormalTok{)}
        \BuiltInTok{print}\NormalTok{(mean\_of\_means)}
\end{Highlighting}
\end{Shaded}
...
Como acabamos de ver, se quisermos acessar a variável
\texttt{mean\_of\_means} o Python retornará um erro. As variáveis
definidas dentro de uma função tem vida curta: elas existem apenas
dentro da função!
...
\begin{Highlighting}
\textcolor{black}{NameError: name 'mean\_of\_means' is not defined}
\textcolor{black}{}\textcolor{QuartoInternalColor4}{---------------------------------------------------------------------------}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{NameError}\textcolor{QuartoInternalColor2}{                                 Traceback (most recent call last)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{Cell}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor1}{ }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{In[26]}\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{, line 1}\textcolor{QuartoInternalColor2}{}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{----> }\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor3}{1}\textcolor{QuartoInternalColor2}{ }\textcolor{QuartoInternalColor7}{print}\textcolor{QuartoInternalColor2}{(}\textcolor{QuartoInternalColor2}{mean\_of\_means}\textcolor{QuartoInternalColor2}{)}
\textcolor{QuartoInternalColor2}{}\textcolor{QuartoInternalColor4}{NameError}\textcolor{QuartoInternalColor2}{: name 'mean\_of\_means' is not defined}
\end{Highlighting}

No entanto, agora já sabemos o que a palavra-chave \texttt{return} faz
dentro de uma função. Vamos utilizá-la!
...
passada basta colocar essa função dentro de um loop e tacar-lhe pau
nesse carrinho, Marcos!
...
Como exercício para casa, tente fazer alterações nessa função de modo
que ela receba não um valor com o número de amostras, mas uma lista de
números de amostras. A função deve cuspir como resultado a lista
\texttt{means\_of\_means} e não apenas o valor \texttt{mean\_of\_means}.
Pratique!

\section{Documentação}\label{documentauxe7uxe3o}

Uma \textbf{docstring} é uma string no início de uma função, definida
pelo usuário, que serve como documentação do que a função faz. A
docstring vem logo depois da primeira linha que define a função e é
delimitada por aspas triplas, o que permite que a string se estenda por
várias linhas, como vocês devem se lembrar.

Vamos criar como exemplo uma função que printa o tipo do objeto que é
...
Embora opcional, a documentação é uma \textbf{boa prática de
programação}. A menos que você consiga se lembrar qual foi o cardápio do
bandejão na semana passada, sempre documente seu código. Podemos acessar
a documentação de determinada função utilizando o atributo
\texttt{\_\_doc\_\_}.
...
Isso vale também para funções nativas e definidas em outros pacotes do
Python. Por exemplo,
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# documentação da função time() dentro do pacote time}
\BuiltInTok{print}\NormalTok{(time.time.\_\_doc\_\_)}
\end{Highlighting}
\end{Shaded}
...
Em Python, uma função anônima é uma função definida sem nome. Quem
poderia imaginar, não é mesmo?

Enquanto as funções normais são definidas usando a palavra-chave
\texttt{def} em Python, as funções anônimas são definidas usando a
palavra-chave \texttt{lambda}. Portanto, funções anônimas também são
chamadas de \textbf{funções lambda}. A estrutura usual de uma função
lambda é a seguinte:
...
Usualmente a gente precisa de uma função lambda porque precisamos de uma
função rápida por um período de tempo e/ou quando a gente usa técnicas
mais poderosas que possuem funções como argumento, como \texttt{filter}
e \texttt{map}. Vou fazer um exemplo com cada uma delas.
...
\begin{verbatim}
Tempo necessário para a realização dos cálculos utilizando arrays: 0.0126 segundos
Tempo necessário para a realização dos cálculos utilizando listas: 0.2853 segundos

A abordagem de listas demorou 22x mais tempo! Esqueça listas e use arrays ;)
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-6-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-8-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-9-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-10-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-11-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-12-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-13-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-14-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-16-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-20-output-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{visualizacao_files/figure-pdf/cell-25-output-1.pdf}}
