...
O comando `return` é usado para especificar qual será o resultado de uma função. Essa declaração pode ter um comando que é executado e o valor resultante devolvido. Se esse comando não for anexado no corpo da função ou não houver nada além do `return`, a função é devolvida com o valor de `None`. Por exemplo:
