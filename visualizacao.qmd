...
Legal, mas o que significam todas essas coisas? Começamos utilizando a função `subplots()` do `matplotlib.pyplot`, que nos retorna dois objetos aos quais damos os nomes de `fig` e `ax`. Sem entrar nos detalhes de classes e instâncias da lógica de programação orientada a objetos no Python:
...
Vamos construir um exemplo de scatterplot a partir da relação entre horas trabalhadas e PIB per capita para o ano de 2010. Se por um lado um número maior de horas trabalhadas na média pode estar associado a um PIB per capita maior de forma mecânica - mais trabalho levando a mais produção -, por outro lado países mais produtivos e desenvolvidos podem ter legislações que limitem a jornada de trabalho dos indivíduos justamente pelo fato de serem países ricos. Olhar para o gráfico de dispersão nesse caso pode nos dar uma luz em relação a qual efeito prevalece. Atenção: pela escala do PIB per capita, talvez seja melhor olhar para o log do PIB ao invés do número absoluto.
