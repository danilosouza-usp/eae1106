...
Com o dataframe criado, podemos plotar o número médio de horas trabalhadas no Brasil ao longo do tempo usando uma única função: `plot`. Essa função recebe como argumento os valores do eixo `x` (no nosso caso o ano) e do eixo `y` (horas trabalhadas) e nos devolve um gráfico de linha relacionando esses dois conjuntos de dados. Por fim é preciso chamar a função `show()` para mostrar o gráfico que criamos.
