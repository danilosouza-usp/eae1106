...
## base final
df_polh.to_csv('dados/secundario/UNDOC_HomicidioPolicia2015.csv', sep=',', encoding='utf8')
```
...
Nesse caso, percebemos que são X as observações na base, Y variáveis disponíveis e Z valores faltantes. Note que, nesse último caso, fomos nós que levamos a isso, já que o uso do método `dropna()` antes de salvar o dataframe final já corrigiu potenciais problemas, embora tenha nos deixado com uma amostra de apenas Z países. Em seguida, podemos utilizar o método `describe()`, que produz um resumo estatístico das variáveis numéricas presentes no dataframe.

```{python}
HomicidioPolicia2015.describe()
```

Essas estatísticas ajudam a identificar possíveis problemas nos dados e fornecem uma primeira visão sobre a variabilidade das variáveis analisadas. Por exemplo, podemos verificar se existem países com taxas de homicídio excepcionalmente altas ou se há grande heterogeneidade no efetivo policial entre os países da amostra.
...
No nosso caso da relação entre polícia e crime
