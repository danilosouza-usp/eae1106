...
A ideia daqui para frente é trabalhar com os dados da [Penn World Table version 11.0](https://www.rug.nl/ggdc/productivity/pwt/?lang=en), base de dados com informação sobre níveis de renda, produto e produtividade de mais de 180 países durante o período de 1950 a 2023. Esses dados são mantidos pela Universidade de Groningen e atualizados esporadicamente. O arquivo específico com o qual trabalharemos está disponível no Moodle e deve ser baixado para a sua máquina local.
...
Legal, Agora utilizaremos a função `pandas.read_csv` para ler o arquivo `pwt110_selected.csv`. Esse arquivo contém informações selecionadas para os 185 países da amostra. Note que, para além do nome do arquivo que será lido, passamos outros dois argumentos para a função: `sep` e `encoding`. Esse argumentos...
...
O poder das funções `read_` é muito grande. Voltaremos ao tema de como exportar os dados criados a partir das funções `to_` mais adiante nesse capítulo.
...
No entanto, para selecionar um intervalo específico de linhas e colunas, a lógica de indexação que utilizamos com arrays de duas dimensões no NumPy não se aplica. Para tal, precisamos de alguns métodos de DataFrames.
...
Utilizando a função `.iloc` podemos selecionar linhas e colunas com os índices numéricos. E se quisermos, por exemplo, manter todas as linhas mas apenas as colunas `country`, `currency_unit`, `year`, `pop`, `emp` e `rgdpna`? Lembre-se que quando o Python lida com intervalos, na grande maioria das vezes a linguagem inclui o índice de início do intervalo mas exclui o índice final.
...
Uma terceira forma de fazer esse exercício de seleção de uma parte de um DataFrame já criado é através da criação de um novo DataFrame que mantenha apenas as colunas de interesse.
...
```{python}
df_pwt2['decada'] = df_pwt2['ano'].astype(str).str[-2:]
...
![Tipo de união entre duas bases de dados, A e B](images/merge.png){#fig-merge width=800px max-width=100% fig-align="center"}
...
Mas note que existem colunas com o mesmo nome nas bases do Brasil e EUA. Por padrão o pandas renomeia essas colunas com `_x` e `_y`. Vamos renomear essas colunas utilizando dicionários, uma forma alternativa ao que fizemos anteriormente.
