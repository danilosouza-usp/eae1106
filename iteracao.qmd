...
4. Escreva um _loop_ que parta de um número inteiro positivo e retorne ao fim do código a quantidade de dígitos pares que existem no número inicial.

5. O último teorema de _Fermat_ diz que não há nenhum número inteiro positivo _a,b,c_ tal que 
   $$ a^n + b^n = c^n $$
...
6. (Prova de 2026) Comece importando a biblioteca _NumPy_. Defina a `seed` do subpacote `random` do _NumPy_ para $12345$ e então simule 10.000 lançamentos de um dado justo utilizando a função `randint` desse mesmo subpacote. Utilize iteração definida e expressões condicionais para criar o objeto `faces` em que o primeiro elemento é igual ao número de sorteios do número 1, o segundo elemento é o número de sorteios do número 2 até o sexto elemento como o número de sorteios do número 6. Dica: caso você não saiba como essa função funciona, utilize o código `print(numpy.random.randint.__doc__)` para olhar sua documentação. 
