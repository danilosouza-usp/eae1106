...
São três as principais formas de armazenar um valor, um pedaço de informação, no Python: um número, um elemento textual ou uma variável _booleana_. No caso de elementos numéricos, são dois os tipos de dados: `int`, que representa um número inteiro, e `float`, que é um número que possui uma parte inteira mas também uma parte decimal. Elementos textuais são sempre armazenados sob a forma de um `str`, que é a abreviação de _string_. Por fim, um dado do tipo _booleano_ é um dado que pode ser assumir apenas dois valores: **True** e **False**.  

## Objetos básicos

Em Python, _tudo_ é um objeto. E um objeto nada mais é do que um elemento dentro da linguagem que possui (i) um tipo, (ii) um valor atribuído, (iii) um conjunto de métodos associados.
...
3. Considere as duas listas abaixo. Escreva um programa que as converta em um dicionário em que os itens de `keys` correspondam às chaves do dicionários e os itens de `values` os valores.
