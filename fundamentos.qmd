...
Apesar de útil, o terminal ainda exige que as instruções sejam dadas comando a comando, de forma relativamente limitada e pouco reutilizável. Para resolver problemas mais gerais e automatizar tarefas de forma sistemática, é necessário um nível adicional de abstração.

## Linguagens de programação 

Para atingir esse nível adicional de abstração, precisamos de um meio de comunicar instruções ao computador de maneira precisa e sem ambiguidades. Esse meio são as linguagens de programação, que definem regras formais para descrever operações, decisões e cálculos que o computador pode executar. Para entender por que essas linguagens precisam ser tão rigorosas, é útil compará-las com outras formas de linguagem usadas pelos seres humanos.

As linguagens naturais são os idiomas que as pessoas falam, como português, inglês e espanhol. Elas não foram criadas deliberadamente, mas se desenvolveram ao longo do tempo, incorporando ambiguidades, exceções e variações de significado. As linguagens formais, por outro lado, são linguagens criadas pelas pessoas para aplicações específicas. Por exemplo, a notação usada pelos matemáticos é uma linguagem formal adequada para representar relações entre números e símbolos; químicos utilizam linguagens formais para representar a estrutura de moléculas. Um outro tipo de linguagem formal, essencial para este curso, são as linguagens de programação, que foram projetadas para comunicar instruções a um computador de maneira inequívoca.
