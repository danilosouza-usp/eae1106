...
A construção de um gráfico de barras é bastante parecida, apenas com a substituição de `geom_line()` por `geom_col()`. Nesse caso vou adicionar algumas outras personalizações, como a definição de um tema padrão^[A lista oficial de temas pré-definidos presentes no ggplot2 pode ser acessada [aqui](https://ggplot2.tidyverse.org/reference/ggtheme.html)], de limites para o eixo y (`scale_y_continuous()`), e da cor das barras através da inclusão do parâmetro `fill` dentro de `geom_col()`.
...
ggplot(df, aes(x=ano,y=razao_part)) +
  ...
  labs(title='Evolução da razão de participação no Brasil',
       x='Ano',
       y=' ')
```
