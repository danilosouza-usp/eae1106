...
Além do tipo e da identidade, objetos também são compostos por informações chamadas atributos. Um atributo é simplesmente uma característica associada ao objeto. Dependendo do tipo do objeto, esses atributos podem guardar informações úteis sobre seu estado ou sua estrutura. Em Python, qualquer nome acessado após o operador ponto (`.`) é denominado um atributo do objeto que aparece à esquerda desse operador. 
...
Uma classe reúne, em uma única estrutura, um conjunto de dados e métodos responsáveis por manipular esses dados. Cada objeto criado a partir da classe possui seus próprios dados, mas compartilha os mesmos comportamentos definidos pela classe. Por exemplo, números inteiros pertencem à classe `int`, listas pertencem à classe `list` e dataframes pertencem à classe `DataFrame`. Embora tenhamos até agora trabalhado apenas com objetos fornecidos pela própria linguagem ou por bibliotecas externas, o Python também permite que criemos nossos próprios tipos de objetos através da definição de novas classes.

### Classes personalizadas

Por exemplo, suponha que desejamos representar municípios brasileiros em um programa a partir dos dados do Censo Populacional de 2022. Cada município possui características como nome, estado ao qual pertence, área total e população no ano de coleta do censo. Em vez de armazenar essas informações em estruturas separadas para cada novo objeto, podemos criar uma classe que reúna todos esses elementos como atributos do objeto criado. 

Para criar uma nova classe utilizamos a palavra chave `class` em conjunto com uma estrutura padrão de métodos e parâmetros, em especial o método `__init__` e o parâmetro `self`.

- O método `__init__` é o primeiro e obrigatório. Não entrarei muito no detalhe, mas esse método é responsável por iniciar a classe e permitir que outros métodos sejam implementados nessa estrutura a partir dos parametros iniciados em `__init__`.

- O parâmetro `self` funciona como um prefixo necessário dentro da sintaxe de classes no Python. Para além da presença em `__init__`, todo novo método criado dentro da classe deve ter `self` como primeiro argumento.
...
O pacote `dataclasses` é uma alternativa interessante para reduzir a carga de sintaxe na definição de classes e tornar suas personalizações mais intuitivas. No caso da classe `Municipio` definida anteriormente, podemos utilizar a função `dataclass` e o decorador `@dataclass` na linha exatamente anterior à definição da classe para reescrever o código como:
...
Nesse caso, não é preciso definir o método `__init__` e encher o início da classe com um monte de `self`. Além disso, é possível definir de cara o tipo de dado associado a cada atributo do objeto. Finalmente, nada muda quando da necessidade de definir novos métodos.
